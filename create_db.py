...
mycursor.execute("CREATE TABLE employee_account(employeeId INTEGER(10) NOT NULL AUTO_INCREMENT, age INTEGER(10), firstName VARCHAR(50),  lastName VARCHAR(50), gender VARCHAR(2),  dateOfBirth DATE, roleId INTEGER(10), PRIMARY KEY (employeeId), FOREIGN KEY (roleId) REFERENCES employee_roles(roleId))")
...
mycursor.execute("CREATE TABLE archived_state(caseId INTEGER(10), archivedState VARCHAR(10), archiveNumber INTEGER(10), archivedDate DATE, dateToBeDestroyed DATE, PRIMARY KEY (archiveNumber), FOREIGN KEY (caseId) REFERENCES case_data(caseId))")
